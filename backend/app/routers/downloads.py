"""
TubeVault – Downloads Router v1.4.0
...
@router.post("/festhaenger-befreien")
async def release_stuck_downloads():
    """Festhängende Downloads (status=active ohne laufenden Worker) zurück in Warteschlange.
    Läuft auch automatisch beim Worker-Restart. API-Pfad ist der offizielle Weg zum manuellen Triggern."""
    from app.services.job_service import job_service
    count = await job_service.requeue_many(
        statuses=["active"], job_type="download", reset_retry=False,
    )
    return {"befreit": count}

# Alias für API-Kompatibilität (deprecated — bitte /festhaenger-befreien nutzen)
@router.post("/fix-stale", include_in_schema=False)
async def fix_stale_downloads_alias():
    return await release_stuck_downloads()
