"""
TubeVault – Subscriptions Router v1.6.0
...
async def get_channel_problem_videos(channel_id: str):
    """Geparkte Downloads eines Kanals (dauerhafte Fehler wie 'unavailable',
    'private', 'members-only', oder Retries erschöpft).
    Wird im Frontend pro Abo als 'Problem-Videos'-Sektion angezeigt.
    Pro Eintrag: job_id, video_id, title, thumbnail, Fehlermeldung, Datum.
    Reaktivierung über POST /api/downloads/{job_id}/retry."""
    videos = await rss_service.get_problem_videos(channel_id)
...
async def get_channel_filesystem(channel_id: str):
    ...
    HOST_DATA = os.getenv("TUBEVAULT_HOST_DATA", "/mnt/data/tubevault/data")
