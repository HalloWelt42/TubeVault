"""
TubeVault – Import Service v1.9.20
© HalloWelt42 – Private Nutzung

Smart-Scan für eigene Videos: info.json-Parsing, NFO-Parsing, Thumbnail-Erkennung,
...
class ImportService:
    ...
    async def _analyze_video(
        ...
        """Einzelnes Video analysieren: info.json → NFO → Companion → Matching."""
        ...
        # ── .info.json (Goldstandard — hat alles) ──
        info_json = self._find_and_parse_info_json(base)
        ...
        # ── Metadaten zusammenbauen (info.json > NFO > Dateiname) ──
        ...
        yt_id = None
        info_json_found = info_json is not None
        view_count = None
        like_count = None
        description_text = description

        # info.json hat höchste Priorität
        if info_json:
            title = info_json.get("title", title)
            yt_id = info_json.get("youtube_id")
            channel_name = info_json.get("channel", channel_name)
            duration = info_json.get("duration")
            resolution = info_json.get("resolution")
            codec = info_json.get("codec")
            date_added = info_json.get("date_added")
            view_count = info_json.get("view_count")
            like_count = info_json.get("like_count")
            if info_json.get("description"):
                description_text = info_json["description"]

        # NFO als Fallback für fehlende Felder
        if nfo_data:
            title = title if info_json else nfo_data.get("title", title)
            duration = duration or nfo_data.get("duration")
            resolution = resolution or nfo_data.get("resolution")
            codec = codec or nfo_data.get("codec")
            date_added = date_added or nfo_data.get("date_added")
        ...
        # ── YouTube-ID aus Dateiname (wenn info.json keine hatte) ──
        if not yt_id:
        ...
        return {
            ...
            "info_json_found": info_json_found,
            "thumbnail_found": thumbnail,
            "description_found": description_text is not None,
    ...
    async def get_own_videos(
        ...
        conditions = ["v.source IN ('local', 'imported')", "v.status = 'ready'", "v.file_path IS NOT NULL", "v.file_path != ''", "v.file_size > 0"]
        params = []
    ...
    async def _run_scan_job(
        ...
        for dirpath, dirnames, filenames in os.walk(scan_path):
            ...
            for fname in sorted(filenames):
                ...
                # Channel-Ordner: erste 2 Ebenen (z.B. youtube/ARTE statt youtube/ARTE/Videotitel)
                parts = Path(folder_name).parts
                channel_folder = str(Path(*parts[:2])) if len(parts) >= 2 else folder_name
                ...
                batch.append((
                    session_id, str(f), info["filename"], folder_name, channel_folder,
                    ...
                    1 if info.get("info_json_found") else 0,
        ...
        for row in batch:
            await db.execute(
                """INSERT INTO scan_staging (
                    session_id, file_path, filename, folder_name, channel_folder,
                    title, channel_name, duration, file_size,
                    resolution, codec, youtube_id, is_portrait,
                    nfo_found, info_json_found, thumbnail_path, description_text, subtitles_found,
                    match_type, match_id, match_title, match_channel,
    ...
    async def get_scan_results(
        ...
        for r in rows:
            ...
            d["_fileExists"] = os.path.exists(d["file_path"]) if d.get("file_path") else False
            # JSON-Felder parsen
            try:
        ...
        # Ordner-Liste für Filter (Kanal-Ebene, nicht Video-Ordner)
        folders = await db.fetch_all(
            """SELECT COALESCE(channel_folder, folder_name) as folder_name,
                      COUNT(*) as count,
                      SUM(CASE WHEN decision IS NOT NULL THEN 1 ELSE 0 END) as decided
               FROM scan_staging WHERE session_id = ?
               GROUP BY COALESCE(channel_folder, folder_name)
               ORDER BY COALESCE(channel_folder, folder_name)""",
            (session_id,)
        )
    ...
    async def execute_decisions(self, session_id: int) -> dict:
        ...
        for r in rows:
            r = dict(r)  # sqlite3.Row → dict (für .get() Support)
            try:
                decision = r["decision"]
                ...
                # Entscheidung zurücksetzen damit Item wieder bearbeitbar ist
                try:
                    await db.execute(
                        "UPDATE scan_staging SET decision = NULL, decision_at = NULL WHERE id = ?",
                        (r["id"],)
                    )
                except Exception:
                    pass
        ...
        # Session-Status: nur 'executed' wenn alles abgearbeitet
        remaining = await db.fetch_val(
            "SELECT COUNT(*) FROM scan_staging WHERE session_id = ? AND (decision IS NULL OR decision = '')",
            (session_id,)
        )
        new_status = "done" if remaining and remaining > 0 else "executed"
        await db.execute(
            "UPDATE scan_sessions SET status = ? WHERE id = ?",
            (new_status, session_id)
        )

        # FTS5 Index neu aufbauen (bulk, effizienter als per-Video)
        if results["linked"] + results["imported"] + results["replaced"] > 0:
            try:
                await db.execute("INSERT INTO videos_fts(videos_fts) VALUES('rebuild')")
                logger.info(f"FTS5 Index rebuilt nach Execute ({results['linked']+results['imported']+results['replaced']} Videos)")
            except Exception as e:
    ...
    def _move_video_to_library(self, src_path: str, video_id: str) -> Path:
        ...
        try:
            ...
            # Read-only Quelle: kopieren statt verschieben
            shutil.copy2(str(src), str(dest))
        logger.info(f"Video übernommen: {src.name} → {dest.name}")
        return dest
        ...
        for ext in SUBTITLE_EXTENSIONS:
            for candidate in base.parent.glob(base.name + "*" + ext):
                ...
                try:
                    ...
                    logger.info(f"Untertitel übernommen: {candidate.name} → {dest.name}")
                except Exception as e:
    ...
    def _delete_with_companions(self, file_path: str):
        ...
        Fehler bei read-only Dateisystemen werden ignoriert.
        """
        fp = Path(file_path)
        base = fp.with_suffix("")
        ...
        try:
            ...
            logger.warning(f"Löschen fehlgeschlagen (read-only?): {fp}: {e}")
    ...
    def _delete_companions(self, file_path: str):
        ...
        for suffix in THUMB_PATTERNS:
            for ext in IMAGE_EXTENSIONS:
                _safe_unlink(base.parent / (base.name + suffix + ext))
        for ext in IMAGE_EXTENSIONS:
            _safe_unlink(base.with_suffix(ext))
        ...
        for pattern in [".info.json", ".meta.json"]:
            _safe_unlink(base.parent / (base.name + pattern))
    ...
    # ─── yt-dlp .info.json Parsing ──────────────────────────────

    def _find_and_parse_info_json(self, base: Path) -> Optional[dict]:
        """yt-dlp .info.json suchen und parsen — Goldstandard für Metadaten."""
        candidates = [
            base.parent / (base.name + ".info.json"),
        ...
        for p in candidates:
            if p.exists():
                try:
                    ...
                    return self._parse_info_json(data)
                except Exception as e:
                    logger.warning(f"info.json Parse-Fehler {p}: {e}")
        return None

    def _parse_info_json(self, data: dict) -> dict:
        """yt-dlp JSON-Metadaten in einheitliches Format konvertieren."""
        result = {}

        # YouTube-ID (zuverlässigste Quelle)
        vid_id = data.get("id") or data.get("display_id")
        ...
        # Kanal
        channel = data.get("uploader") or data.get("channel") or data.get("creator")
        if channel:
        ...
        if upload_date and len(upload_date) == 8:
            result["date_added"] = f"{upload_date[:4]}-{upload_date[4:6]}-{upload_date[6:8]}"
    ...
    def _extract_youtube_id(self, filename: str) -> Optional[str]:
        ...
        Patterns (Priorität):
        1. [dQw4w9WgXcQ]           — yt-dlp mit Klammern
        2. (dQw4w9WgXcQ)           — Alternative Klammern
        3. -dQw4w9WgXcQ.f137.mp4   — yt-dlp Format-Suffix
        4. -dQw4w9WgXcQ.mp4        — yt-dlp Standard
        5. _dQw4w9WgXcQ.mp4        — Unterstrich-Trenner
        6. dQw4w9WgXcQ.mp4         — Nur ID als Name
        """
        ...
        # -ID.f137.ext Pattern (yt-dlp Format-Downloads)
        m = re.search(r'-([a-zA-Z0-9_-]{11})\.f\d+\.', filename)
        if m:
            return m.group(1)
        ...
        # _ID.ext Pattern (Unterstrich-Trenner)
        m = re.search(r'_([a-zA-Z0-9_-]{11})\.[a-zA-Z0-9]+$', filename)
        if m:
            return m.group(1)

        # ID.ext Pattern (nur ID als Dateiname)
        m = re.match(r'^([a-zA-Z0-9_-]{11})\.[a-zA-Z0-9]+$', filename)
        if m:
            return m.group(1)
