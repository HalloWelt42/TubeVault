...
RATE_LIMIT_KEYWORDS = (
    ...
    "detected as a bot", "do not open an issue",
)
...
class RateLimiter:
    ...
    def __init__(self):
        ...
        self._yt_bot_detected: bool = False # Bot-Erkennung aktiv
        self._yt_bot_until: float = 0.0    # Bot-Cooldown bis Timestamp
    ...
    def error(self, category: str, error_msg: str = "") -> float:
        ...
        if is_rate_limit:
            ...
            # Bot-Erkennung: 1h Sperre
            is_bot = error_msg and ("detected as a bot" in error_msg.lower() or "do not open an issue" in error_msg.lower())
            if is_bot:
                self._yt_bot_detected = True
                self._yt_bot_until = time.time() + 3600  # 1 Stunde
                logger.warning(f"YouTube BOT-ERKENNUNG '{category}': 1h Sperre aktiv | {error_msg[:120]}")
            else:
    ...
    def is_youtube_healthy(self) -> bool:
        ...
        if self._yt_bot_detected and time.time() < self._yt_bot_until:
            return False
        if self._yt_bot_detected and time.time() >= self._yt_bot_until:
            self._yt_bot_detected = False  # Auto-Reset nach 1h
    ...
    def is_bot_detected(self) -> bool:
        """Gibt True zurück wenn Bot-Erkennung aktiv ist (1h Fenster)."""
        if self._yt_bot_detected and time.time() >= self._yt_bot_until:
            self._yt_bot_detected = False
        return self._yt_bot_detected
