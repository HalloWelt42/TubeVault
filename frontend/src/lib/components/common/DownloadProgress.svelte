...
<script>
  ...
  // Fallback-Phasen (adaptive 5-Phasen) — nur verwendet, wenn Backend keine phases schickt
  // UND wir noch keine Infos haben. Sobald Backend eine phases-Liste sendet (progressive=3,
  // adaptiv=5, audio-only=3), wird diese respektiert.
  ...
  function inferPhases() {
    // Wenn wir weder phases noch Infos über den Typ haben, bei Done/Error eine
    // minimale 1-Phase-Anzeige liefern — lieber knapp und korrekt als 5 falsche Phasen.
    const stage = data.stage || '';
    const status = data.status || '';
    if (stage === 'done' || status === 'done') {
      return [{ id: 'done', label: 'Abgeschlossen', start: 0, end: 1, color: '#4CAF50', status: 'done' }];
    }
    if (stage === 'error' || status === 'error') {
      return [{ id: 'error', label: 'Fehler', start: 0, end: 1, color: '#F44336', status: 'active' }];
    }
    if (stage === 'cancelled' || status === 'cancelled') {
      return [{ id: 'cancelled', label: 'Abgebrochen', start: 0, end: 1, color: '#9E9E9E', status: 'active' }];
    }
    if (stage === 'parked' || status === 'parked') {
      return [{ id: 'parked', label: 'Geparkt', start: 0, end: 1, color: '#FF9800', status: 'active' }];
    }

    // Laufend: adaptive 5-Phasen als Annahme (wird überschrieben sobald Backend
    // die korrekte phases-Liste mitsendet).
