...
<script>
  ...
  async function pollSystemStatus() {
    try {
      ...
      botDetected = s.services?.youtube?.bot_detected ?? false;
  ...
  function handleProgressMessage(msg) {
    ...
    // Terminal-Status aus dem Progress-Broadcast SOFORT in jobs[] übernehmen,
    // damit der Job sofort aus activeJobs rausfliegt (statt auf den job_update
    // zu warten, der evtl. verspätet/verloren gehen könnte).
    const terminal = ['done', 'error', 'cancelled', 'parked'];
    const status = msg.status || msg.stage;
    if (terminal.includes(status)) {
      const idx = jobs.findIndex(j => j.id === id);
      if (idx >= 0 && jobs[idx].status !== status) {
        jobs[idx] = { ...jobs[idx], status, progress: msg.progress ?? jobs[idx].progress };
      }
      // Nach kurzer Verzögerung Liste frisch laden — holt completed_at etc.
  ...
  function handleJobUpdate(msg) {
    ...
    if (['done', 'error', 'cancelled', 'parked'].includes(updated.status)) {
      setTimeout(loadJobs, 500);
    }
  }
...
<style>
  ...
  .led-bot { background: #f5c542; box-shadow: 0 0 6px #f5c542; animation: led-pulse 1.5s ease-in-out infinite; }
